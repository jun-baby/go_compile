...
func addStrings(stringTable *dataTable[stringID, string], b batch) error {
	if !b.isStringsBatch() {
		return fmt.Errorf("src_internal error: addStrings called on non-string batch")
	}
...
func addStacks(stackTable *dataTable[stackID, stack], b batch) error {
	if !b.isStacksBatch() {
		return fmt.Errorf("src_internal error: addStacks called on non-stacks batch")
	}
	r := bytes.NewReader(b.data)
...
func addCPUSamples(samples []cpuSample, b batch) ([]cpuSample, error) {
	if !b.isCPUSamplesBatch() {
		return nil, fmt.Errorf("src_internal error: addStrings called on non-string batch")
	}
	r := bytes.NewReader(b.data)
...
func parseFreq(b batch) (frequency, error) {
	if !b.isFreqBatch() {
		return 0, fmt.Errorf("src_internal error: parseFreq called on non-frequency batch")
	}
	r := bytes.NewReader(b.data)
