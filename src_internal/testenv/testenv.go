...
//
// It is an src_internal package because these details are specific
...
import (
	"bytes"
	"compile/src_internal/cfg"
	"compile/src_internal/platform"
	"errors"
	"flag"
	"fmt"
	"os"
...
func findGOROOT() (string, error) {
	gorootOnce.Do(func() {
		...
		//
		// Since this is src_internal/testenv, we can cheat and assume that the caller
...
// src_internal linking.
func CanInternalLink(withCgo bool) bool {
	return !platform.MustLinkExternal(runtime.GOOS, runtime.GOARCH, withCgo)
}

// MustInternalLink checks that the current system can link programs with src_internal
// linking.
// If not, MustInternalLink calls t.Skip with an explanation.
func MustInternalLink(t testing.TB, withCgo bool) {
	if !CanInternalLink(withCgo) {
		if withCgo && CanInternalLink(false) {
			t.Skipf("skipping test: src_internal linking on %s/%s is not supported with cgo", runtime.GOOS, runtime.GOARCH)
		}
		t.Skipf("skipping test: src_internal linking on %s/%s is not supported", runtime.GOOS, runtime.GOARCH)
	}
}
