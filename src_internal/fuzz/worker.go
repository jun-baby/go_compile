...
const (
	...
	// workerExitCode is used as an exit code by fuzz worker processes after an src_internal error.
	// This distinguishes src_internal errors from uncontrolled panics and other crashes.
	// Keep in sync with src_internal/fuzz.workerExitCode.
	workerExitCode = 70
	...
	for {
		...
		case <-w.termC:
			...
			if exitErr, ok := err.(*exec.ExitError); ok && exitErr.ExitCode() == workerExitCode {
				...
				return fmt.Errorf("fuzzing process exited unexpectedly due to an src_internal failure: %w", err)
			}
		...
		case input := <-w.coordinator.inputC:
			...
			if err != nil {
				...
				if isInternalError {
					// An src_internal error occurred which shouldn't be considered
					// a crash.
					return err
				}
...
type fuzzResponse struct {
	...
	// InternalErr is the error string caused by an src_internal error in the
...
// interrupted.
var errSharedMemClosed = errors.New("src_internal error: shared memory was closed and unmapped")
