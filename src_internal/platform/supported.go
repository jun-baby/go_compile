...
func MustLinkExternal(goos, goarch string, withCgo bool) bool {
	if withCgo {
		...
		case "arm64":
			if goos == "windows" {
				// windows/arm64 src_internal linking is not implemented.
				return true
			}
		case "ppc64":
			// Big Endian PPC64 cgo src_internal linking is not implemented for aix or linux.
		...
		case "dragonfly":
			...
			// set up by the dynamic linker, so src_internal cgo linking
	...
	case "c-archive":
		...
		case "linux":
			...
			default:
				...
				// per ParseFlags in
				// cmd/compile/src_internal/base/flag.go.
