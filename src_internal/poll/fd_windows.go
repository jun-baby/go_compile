...
import (
	"compile/src_internal/race"
	"compile/src_internal/syscall/windows"
	"errors"
	"io"
	"sync"
	"syscall"
...
func execIO(o *operation, submit func(o *operation) error) (int, error) {
	if o.fd.pd.runtimeCtx == 0 {
		return 0, errors.New("src_internal error: polling on unsupported descriptor type")
	}
...
func (fd *FD) Init(net string, pollable bool) (string, error) {
	...
	case "tcp", "tcp4", "tcp6",
		...
		fd.kind = kindNet
	default:
		return "", errors.New("src_internal error: unknown network type " + net)
	}
