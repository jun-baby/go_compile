...
import (
	...
	"compile/src_internal/types/errors"
	"fmt"
	"os"
	"runtime/debug"
	"sort"
...
// Pos 是当前正在处理的源位置，由 Errorf, ErrorfLang、Fatalf 和 Warnf 打印。
var Pos src.XPos
...
// Fatalf reports a fatal error - an cmd_internal problem - at the current line and exits.
// If other errors have already been printed, then Fatalf just quietly exits.
// (The cmd_internal problem may have been caused by incomplete information
// after the already-reported errors, so best to let users fix those and
// try again without being bothered about a spurious cmd_internal error.)
//
// But if no errors have been printed, or if -d panic has been specified,
// Fatalf prints the error as an "compile/cmd_internal compiler error". In a released build,
...
// FatalfAt reports a fatal error - an cmd_internal problem - at pos and exits.
...
//
// But if no errors have been printed, or if -d panic has been specified,
// FatalfAt prints the error as an "compile/cmd_internal compiler error". In a released build,
// it prints an error asking to file a bug report. In development builds, it
// prints a stack trace.
//
...
func FatalfAt(pos src.XPos, format string, args ...interface{}) {
	...
	if Debug.Panic != 0 || numErrors == 0 {
		fmt.Printf("%v: cmd_internal compiler error: ", FmtPos(pos))
