...
type Inline struct {
	...
	// making inlining decisions). See cmd/compile/cmd_internal/inline/inlheur.
	Properties string
...
// prepended.
//
// This differs from the compiler's cmd_internal convention where local functions
...
// IsFuncPCIntrinsic returns whether n is a direct call of cmd_internal/abi.FuncPCABIxxx functions.
func IsFuncPCIntrinsic(n *CallExpr) bool {
	...
	return (fn.Name == "FuncPCABI0" || fn.Name == "FuncPCABIInternal") &&
		fn.Pkg.Path == "compile/cmd_internal/abi"
}
...
// function as uintptr, as returned by cmd_internal/abi.FuncPC{ABI0,ABIInternal}.
//
// n should be a Node of an interface type, as is passed to
// cmd_internal/abi.FuncPC{ABI0,ABIInternal}.
//
...
func FuncPC(pos src.XPos, n Node, wantABI obj.ABI) Node {
	if !n.Type().IsInterface() {
		base.ErrorfAt(pos, 0, "compile/cmd_internal/abi.FuncPC%s expects an interface value, got %v", wantABI, n.Type())
	}
	...
	if fn := IsIfaceOfFunc(n); fn != nil {
		...
		abi := fn.ABI
		if abi != wantABI {
			base.ErrorfAt(pos, 0, "compile/cmd_internal/abi.FuncPC%s expects an %v function, %s is defined as %v", wantABI, wantABI, name.Sym().Name, abi)
		}
	...
	if wantABI != obj.ABIInternal {
		base.ErrorfAt(pos, 0, "compile/cmd_internal/abi.FuncPC%s does not accept func expression, which is ABIInternal", wantABI)
	}
