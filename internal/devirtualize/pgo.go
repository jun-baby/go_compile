...
//
//	func foo(fn func()) {
//		if cmd_internal/abi.FuncPCABIInternal(fn) == cmd_internal/abi.FuncPCABIInternal(Concrete) {
//			Concrete()
//		} else {
//			fn()
//		}
...
func maybeDevirtualizeFunctionCall(p *pgo.Profile, fn *ir.Func, call *ir.CallExpr) (ir.Node, *ir.Func, int64) {
	...
	// are generated by cmd/compile/cmd_internal/reflectdata.genhash.
...
func rewriteFunctionCall(call *ir.CallExpr, curfn, callee *ir.Func) ir.Node {
	...
	//
	// fnPC := cmd_internal/abi.FuncPCABIInternal(fn)
	// concretePC := cmd_internal/abi.FuncPCABIInternal(concrete)
	//
	// if fnPC == concretePC {
