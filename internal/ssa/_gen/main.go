...
func genOp() {
	...
	fmt.Fprintln(w, "\"cmd/cmd_internal/obj\"")
	for _, a := range archs {
		if a.pkg != "" {
			fmt.Fprintf(w, "%q\n", a.pkg)
		}
	...
	for _, a := range archs {
		...
		for i, r := range a.regnames {
			num[r] = int8(i)
			pkg := a.pkg[len("cmd/cmd_internal/obj/"):]
			var objname string // name in cmd/cmd_internal/obj/$ARCH
			switch r {
			case "SB":
				// SB isn't a real register.  cmd/cmd_internal/obj expects 0 in this case.
				objname = "0"
			case "SP":
				objname = pkg + ".REGSP"
			case "g":
