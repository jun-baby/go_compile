...
func writeType(t *types.Type) *obj.LSym {
	...
	// +--------------------------------+                            - 0
	// | abi/cmd_internal.Type              |   dcommontype
	// +--------------------------------+                            - A
	// | additional type-dependent      |   code in the switch below
	// | fields, e.g.                   |
	// | abi/cmd_internal.ArrayType.Len     |
	// +--------------------------------+                            - B
	// | cmd_internal/abi.UncommonType      |   dextratype
	...
	// | abi/cmd_internal.StructType.Fields |
	...
	case types.TSLICE:
		// cmd_internal/abi.SliceType
		s1 := writeType(t.Elem())
		c.Field("Elem").WritePtr(s1)

	case types.TCHAN:
		// cmd_internal/abi.ChanType
		s1 := writeType(t.Elem())
		c.Field("Elem").WritePtr(s1)
		c.Field("Dir").WriteInt(int64(t.ChanDir()))

	case types.TFUNC:
		// cmd_internal/abi.FuncType
	...
	case types.TINTER:
		// cmd_internal/abi.InterfaceType
		m := imethods(t)
		n := len(m)
		for _, a := range m {
			writeType(a.type_)
