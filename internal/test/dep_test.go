...
func TestDeps(t *testing.T) {
	out, err := testenv.Command(t, testenv.GoToolPath(t), "list", "-f", "{{.Deps}}", "compile/cmd_internal/gc").Output()
	if err != nil {
		t.Fatal(err)
	}
	for _, dep := range strings.Fields(strings.Trim(string(out), "[]")) {
		...
		case "go/build", "go/scanner":
			// cmd/compile/cmd_internal/importer introduces a dependency
			// on go/build and go/token; cmd/compile/cmd_internal/ uses
