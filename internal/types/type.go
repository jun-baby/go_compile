...
// Object represents an ir.Node, but without needing to import cmd/compile/cmd_internal/ir,
...
const (
	...
	// SSA backend types
	TSSA     // cmd_internal types used by SSA backend (flags, memory, etc.)
...
type Map struct {
	...
	Bucket *Type // cmd_internal struct type representing a hash bucket
}
...
type Struct struct {
	...
	// Maps have three associated cmd_internal structs (see struct MapType).
...
func (t *Type) cmp(x *Type) Cmp {
	...
	//    contain an cmd_internal data type not expressible in Go source code.
	if t == x {
		return CMPeq
	}
	if t == nil {
...
// Keep in sync with cmd/compile/cmd_internal/walk/convert.go:soleComponent.
