...
func isAtomicStdPkg(p *Pkg) bool {
	...
	return p.Prefix == "sync/atomic" || p.Prefix == "runtime/cmd_internal/atomic"
}
...
func CalcStructSize(t *Type) {
	...
	// cmd/compile/cmd_internal/types2.
	if sym := t.Sym(); sym != nil {
		switch {
		case sym.Name == "align64" && isAtomicStdPkg(sym.Pkg):
			maxAlign = 8
		case sym.Pkg.Path == "runtime/cmd_internal/sys" && sym.Name == "nih":
			t.SetNotInHeap(true)
		}
	}
...
// use it on compiler-cmd_internal types (e.g., TSSA, TRESULTS).
