...
func (ctxt *Link) DwarfAbstractFunc(curfn Func, s *LSym) {
	...
	if absfn.Size != 0 {
		ctxt.Diag("compile/cmd_internal error: DwarfAbstractFunc double process %v", s)
	}
	if s.Func() == nil {
		s.NewFuncInfo()
	}
...
func (ft *DwarfFixupTable) SetPrecursorFunc(s *LSym, fn Func) {
	if _, found := ft.precursor[s]; found {
		ft.ctxt.Diag("compile/cmd_internal error: DwarfFixupTable.SetPrecursorFunc double call on %v", s)
	}
...
func (ft *DwarfFixupTable) ReferenceChildDIE(s *LSym, ridx int, tgt *LSym, dclidx int, inlIndex int) {
	...
	if len(sf.doffsets) > 0 {
		...
		if !found {
			ft.ctxt.Diag("compile/cmd_internal error: DwarfFixupTable.ReferenceChildDIE unable to locate child DIE offset for dclIdx=%d src=%v tgt=%v", dclidx, s, tgt)
		}
	} else {
...
func (ft *DwarfFixupTable) RegisterChildDIEOffsets(s *LSym, vars []*dwarf.Var, coffsets []int32) {
	...
	if len(vars) != len(coffsets) {
		ft.ctxt.Diag("compile/cmd_internal error: RegisterChildDIEOffsets vars/offsets length mismatch")
		return
	}
...
func (ft *DwarfFixupTable) processFixups(slot int, s *LSym) {
	...
	for _, f := range sf.fixups {
		...
		if !dfound {
			ft.ctxt.Diag("compile/cmd_internal error: DwarfFixupTable has orphaned fixup on %v targeting %v relidx=%d dclidx=%d", f.refsym, s, f.relidx, f.dclidx)
		}
	}
}
...
func (ft *DwarfFixupTable) AbsFuncDwarfSym(fnsym *LSym) *LSym {
	...
	ft.ctxt.Diag("compile/cmd_internal error: AbsFuncDwarfSym requested for %v, not seen during inlining", fnsym)
	return nil
}
...
func (ft *DwarfFixupTable) Finalize(myimportpath string, trace bool) {
	...
	if ft.ctxt.InParallel {
		ft.ctxt.Diag("compile/cmd_internal error: DwarfFixupTable.Finalize call during parallel backend")
	}
	...
	for _, s := range fns {
		...
		slot, found := ft.symtab[absfn]
		if !found {
			ft.ctxt.Diag("compile/cmd_internal error: DwarfFixupTable.Finalize orphan abstract function for %v", s)
		} else {
